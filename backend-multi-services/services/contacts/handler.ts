...
export const fetch: APIGatewayProxyHandler = async event => {
  ...
  try {
    ...
    console.log('connectedUser', JSON.stringify(connectedUser, null, 2))
...
export const changeContactEmail: APIGatewayProxyHandler = async event => {
  const cognitoIdentityClient = new CognitoServiceProviderClient()
  try {
    const connectedUser = await cognitoIdentityClient.getUser(
      cognitoIdentityClient.getEventAuthValues(event)
    ...
    if (parsedData.email !== connectedUser.email) {
      const Contacts = new ContactsController(contactsClient)

      const contactEmail = Contacts.updateAllFromParams({
        email: connectedUser.email,
      }, {
        email: parsedData.email,
      }, null)

      const cognitoUserEmail = cognitoIdentityClient.updateUserAttributes(
        {
          cognitoSub: connectedUser['sub'],
          userPoolId: process.env.USER_POOL,
        },
      ...
      const promises = await Promise.all([contactEmail, cognitoUserEmail])
...
export const sendIntroEmail: APIGatewayProxyHandler = async event => {
  try {
    const parsedData = new BodyParserTransformer().transform(event.body)

    if (!parsedData.email || !parsedData.firstName || !parsedData.lastName || !parsedData.locale) {
    ...
    if (parsedData.locale === 'fr') {
      emailTitle = 'Conecto Entrepreneur - Nouvelle plateforme'
      urlTo = process.env.CORS_ORIGIN_1
      templateName = process.env.MANDRILL_TEMPLATE_INTRO_FR_NAME
    } else {
      emailTitle = 'Conecto Contractor - New platform'
      urlTo = process.env.CORS_ORIGIN_2
      templateName = process.env.MANDRILL_TEMPLATE_INTRO_EN_NAME
    }
