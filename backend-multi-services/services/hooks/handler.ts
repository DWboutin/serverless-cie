...
import geoDistance from '../../src/Helpers/geoDistance'
...
export const onNewDeal: APIGatewayProxyHandler = async event => {
  ...
  try {
    ...
    if (dealInfo.custom_fields.notified !== 1) {
      /**
      ...
      const companiesPromises = companiesQuery.data.items.map(async (company) => {
        ...
        companyAddresses.Items.forEach((address) => {
          const geoDistanceKm = geoDistance(address.latitude, address.longitude, dealInfo.custom_fields.latitude, dealInfo.custom_fields.longitude)

          if (geoDistanceKm <= address.radius) {
            mustNotify = true
          }
        })
      ...
      const companies = await Promise.all(companiesPromises).then((result) => {
        return result.filter(i => Boolean(i))
      })
      ...
      if (dealExists.Count === 0) {
        ...
        await dynamoDb.put({
          ...
          Item: {
            ...
            jobInfo: dealInfo.custom_fields.jobInfo,
      ...
      companies.forEach(company => {
        ...
        const companyPromise = new Promise(async (resolve, reject) => {
          ...
          if (companyContactsQuery.data.items.length > 0) {
            ...
            const companyContactsPromises = companyContactsQuery.data.items.map(async companyContact => {
            ...
            await Promise.all(companyContactsPromises)
          }
...
export const manualNewDeal: APIGatewayProxyHandler = async event => {
  const cognitoIdentityClient = new CognitoServiceProviderClient()
  logger('Hook On New Deal', 'generic', 'begin')
  try {
    // From Zapier
    const parsedData = new BodyParserTransformer().transform(event.body)
    const { companiesIds, contactId, dealId } = parsedData
    ...
    console.log('companiesIds.join(\',\')', companiesIds.join(','))

    const companiesQuery = await Contacts.searchFor({
      is_organization: true,
      ids: companiesIds.join(','),
    })

    const companies = companiesQuery.data.items

    console.log('CCCcompanies', companies)
    ...
    companies.forEach(company => {
      ...
      const companyPromise = new Promise(async (resolve, reject) => {
        ...
        console.log('companyContactsQuery.data.items', companyContactsQuery.data.items)
        console.log('companyContactsQuery.data.items.length', companyContactsQuery.data.items.length)
        ...
        if (companyContactsQuery.data.items.length > 0) {
          ...
          companyContactsQuery.data.items.forEach(async companyContact => {
            ...
            console.log('companyContactsQuery.data.items.forEach')
            console.log('---------------------------------------')
