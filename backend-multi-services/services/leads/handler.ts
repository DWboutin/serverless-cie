...
export const update: APIGatewayProxyHandler = async event => {
  logger('Lead Update', 'generic', 'begin')
  try {
    const validator = new DataValidator(LeadCreationSchema)
    const leadTransformer = new LeadCreationTransformer([
      'preferredContactMethod',
    ...
    const encryptedLeadId = decodeURIComponent(
      event.pathParameters.encryptedLeadId
    )
    const leadId = parseInt(cryptr.decrypt(encryptedLeadId))
    const parsedData = new BodyParserTransformer().transform(event.body)
    ...
    await Leads.update(leadId, {
      custom_fields: {
        preferredContactMethod: parsedData.preferredContactMethod,
        preferredContactTime: parsedData.preferredContactTime,
      }
    })
